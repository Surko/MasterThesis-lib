 ...
 * the operator on parens population and make the child population.
 * </p>
 * There are few instructions when implementing {@link #execute(IPopulation, IPopulation)} method 
 * for different types of operators :
 * <ul>
 * <li>CrossOver - should apply the crossover with some probability and add the
 * newly created childs into childs array. If crossover isn't applied than 
 * the parents are simply copied into childs array.</li>
 * <li>Mutation - should apply the mutation with some probability on parent. Defaultly it does not
 * add the changed parent into childs (when the childs parameter is null).</li>
 * </ul
 * </p>
 * Other must be implemented methods are {@link #isWekaCompatible()} and {@link #isWekaDependent()} that
 * chooses if the operator can be used with weka. The initialization uses these methods. 
 * @author Lukas Surin
 *
 */
